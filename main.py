...
def encode(images):
    ...
    for img in images:
        ...
        face_encodeImg = face_recognition.face_encodings(img)
        if len(face_encodeImg) > 0:
            encodeImg = face_encodeImg[0]
...
while True:
    ...
    for encodecurr, loc in zip(encodeImg, face):
        ...
        if match[index_BestMatch]:
            ...
            cv2.rectangle(img, (x1, y2-30), (x2, y2),
                          (255, 0, 255), cv2.FILLED)
            cv2.putText(img, name, (x1+8, y2-10),
